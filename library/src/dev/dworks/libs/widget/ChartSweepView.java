...
public class ChartSweepView extends View {
    ...
    private void invalidateLabelTemplate() {
        if (mLabelTemplateRes != 0) {
            ...
            //FIXME paint.setCompatibilityScaling(getResources().getCompatibilityInfo().applicationScale);
            paint.setColor(mLabelColor);
            paint.setShadowLayer(4 * paint.density, 0, 0, Color.BLACK);

            mLabelTemplate = new SpannableStringBuilder(template);
            mLabelLayout = new DynamicLayout(
                    mLabelTemplate, paint, LARGE_WIDTH, Alignment.ALIGN_CENTER, 1f, 0f, false);
            invalidateLabel();
